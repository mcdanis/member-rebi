...
<body class="hold-transition lockscreen" style="background: white !important">
  <?php 
      ...
      $harga = $sistem->getHarga();
      if($_GET['murah']==1){
          $harga = $sistem->getHargaMurah();
      }
   ?>
...
<div class="lockscreen-wrapper">
  ...
  <div class="lockscreen-name alert alert-info">
    ...
    <br>
    <?php if ($_GET['type'] == 'b' || $_GET['murah'] == 1): ?>
      <?php if($_GET['id_akun'] == 0){ ?>
      ...
      <?php } ?>
      <br>
	    <h2>Rp<?= $harga; ?>.<span style="color: gold"><?= $_GET['price'] ?></span></h2>
	    <div style="background: #0096bb;border-radius: 5px;padding: 5px;">
		    ...
		    ?>
        <label style="font-size: 18px;">Untuk mempercepat akses anda klik link ke Wa berikut : <br><a target="_blank" class="gold-color" href="https://wa.me/<?= str_replace_first('0','62',$nowanyadmin); ?>?text=<?= urlencode($pcf) ?>, Transfer senilai %2ARp<?= $harga.'.'.$_GET['price'] ?>%2A%0Aini email saya <?=$_GET['email'] ?>">klik <?= $nowanyadmin ?></a></label>
