<?php 
  ...
  // -------
  $min1 = new DateTime('yesterday');
  $min2 = new DateTime('-2 days');
  $min3 = new DateTime('-3 days');
  $min4 = new DateTime('-4 days');

  $m1 = $sistem->getDay($akun['kode_afiliasi'],strftime('%A, %d %B %Y', $min1->getTimestamp()));
  $m2 = $sistem->getDay($akun['kode_afiliasi'],strftime('%A, %d %B %Y', $min2->getTimestamp()));
  $m3 = $sistem->getDay($akun['kode_afiliasi'],strftime('%A, %d %B %Y', $min3->getTimestamp()));
  $m4 = $sistem->getDay($akun['kode_afiliasi'],strftime('%A, %d %B %Y', $min4->getTimestamp()));
  // -------
...
<section class="content" style="height: auto !important; min-height: 0px !important;">
          ...
          <div class="box box-solid bg-light-blue-gradient" style="background: white !important">
            ...
            <div class="box-body" style="color: black">
               <span class="label label-success"><i class="fa fa-arrow-up"></i> Siap Transfer</span> <small>= Orang yang daftar Ecourse tapi belum transfer</small><br>
                ...
                Investasi Rebi Sekarang <span class="label label-primary">Rp<?php echo $sistem->getHarga().'.000' ?></span><br>
    ...
    <!-- isi konten -->
      <div class="box box-danger">
        ...
        <div class="box-body" style="overflow: scroll;"><i>*Klik Nama Calon Prospek (CP) untuk melihat detail info &amp; <u>APPROVE</u> Jika CP sudah Transfer kepada Anda</i><br>
          <small class="c-primary">*Swipe untuk melihat sisi lainya ke samping dan atas jika kamu menbuka halaman ini di smartphone</small>
        <div class="btn-group" role="group" aria-label="Basic example">
          <!-- <button type="button" class="btn btn-sm btn-danger" onclick="return showByDate('<?= $hari ?>')"><i class="fa fa-calendar"></i> Hari Ini</button> -->
          ...
          <table class="table table-bordered table-striped example1">
            ...
            <tbody>
              <?php 
                ...
                $ost=0;
                $ot=0;
                $no=1; foreach ($sistem->tampil_manual('select * from akun where afiliasi="'.$akun['kode_afiliasi'].'" order by date desc') as $key => $value):
                ...
                $classToday='';
                $today=$value['create_at'];
                if ($hari == $value['create_at']) {
                  $classToday = 'today';
                  $today="HARI INI";
                }
              ?>
     ...
     <script>
        ...
        function showByDate(hari){
          $('input.form-control.input-sm').val(hari);
          $('input.form-control.input-sm').click();
        }
...
var myChart = new Chart(ctx, {
 //chart akan ditampilkan sebagai bar chart
    ...
    data: {
     //membuat label chart
        labels: ['-4 hari', '-3 hari', '-2 hari', 'Kemarin', 'Hari ini'],
        datasets: [{
            label: 'Perbandingan Prospek 5 Hari terakhir',
            //isi chart
            data: [<?= $m4 ?>,<?= $m3 ?>,<?= $m2 ?>,<?= $m1 ?>,<?= $jmlProspekHariIni ?>,],
